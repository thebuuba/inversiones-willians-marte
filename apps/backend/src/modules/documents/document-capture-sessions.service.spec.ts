...
describe('DocumentCaptureSessionsService', () => {
  ...
  it('reserves upload capacity before creating the document', async () => {
    jest.mocked(prisma.documentCaptureSession.updateMany).mockResolvedValue({ count: 1 });
    jest.mocked(prisma.documentCaptureSession.findUnique).mockResolvedValue({
      token: 'active-token',
      clientId: 8,
      createdById: 'user-1',
  ...
  it('throws GoneException when upload capacity cannot be reserved for an existing token', async () => {
    jest.mocked(prisma.documentCaptureSession.updateMany).mockResolvedValue({ count: 0 });
    jest.mocked(prisma.documentCaptureSession.findUnique).mockResolvedValue({
      token: 'full-token',
      clientId: 8,
    } as any);
  ...
  it('rolls back the reserved upload count when document creation fails', async () => {
    jest.mocked(prisma.documentCaptureSession.updateMany).mockResolvedValue({ count: 1 });
    jest
      .mocked(prisma.documentCaptureSession.update)
      .mockResolvedValue({ token: 'active-token' } as any);
    jest.mocked(prisma.documentCaptureSession.findUnique).mockResolvedValue({
      token: 'active-token',
      clientId: 8,
      createdById: 'user-1',
  ...
  it('returns an already closed session without updating it again', async () => {
    ...
    await expect(service.close('closed-token')).resolves.toMatchObject({
      token: 'closed-token',
      closedAt,
    });
