    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 py-6 backdrop-blur-[2px]" onClick={onClose}>
      ...
      >
        ...
        <footer className="flex justify-end gap-3 border-t border-[#edf2ef] px-6 py-4">
          <button className="h-11 rounded-full border border-[#ddebe3] bg-white px-6 text-sm font-bold text-[#173d2c]" disabled={saving} onClick={onClose} type="button">
            Cancelar
          </button>
          <button className="h-11 rounded-full bg-[#285c43] px-6 text-sm font-bold text-white disabled:opacity-60" disabled={saving} type="submit">
