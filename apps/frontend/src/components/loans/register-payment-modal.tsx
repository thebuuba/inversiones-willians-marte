...
export function RegisterPaymentModal({
  ...
  return (
    ...
    >
      <form
        className="flex max-h-[calc(100dvh-3rem)] w-full max-w-[560px] flex-col overflow-hidden rounded-panel border border-border-soft bg-card shadow-[0_28px_80px_rgba(0,0,0,0.26)]"
      ...
      >
        <header className="flex items-start justify-between gap-4 rounded-t-2xl bg-[#f1f8f4] px-6 py-5">
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-text-muted">
              El pago se aplicará a las cuotas pendientes del préstamo.
            </p>
          </div>
          <button
        ...
        <div className="grid flex-1 grid-cols-1 gap-4 overflow-y-auto px-6 py-5 sm:grid-cols-2">
          ...
          {error ? (
            <p className="text-sm font-medium text-state-danger sm:col-span-2">{error}</p>
          ) : null}
        </div>

        <footer className="flex justify-end gap-3 border-t border-border-soft px-6 py-4">
          <button
            className="h-11 rounded-full border border-primary-border bg-card px-6 text-sm font-bold text-text-primary"
            disabled={saving}
            onClick={onClose}
            type="button"
          >
          ...
          <button
            className="h-11 rounded-full bg-primary px-6 text-sm font-bold text-white disabled:opacity-60"
            disabled={saving}
            type="submit"
          >
