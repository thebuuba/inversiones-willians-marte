...
const MAX_CAPTURE_UPLOAD_BYTES = 21 * 1024 * 1024;
...
export async function POST(request: Request, props: { params: Promise<{ token: string }> }) {
  ...
  if (contentLength > MAX_CAPTURE_UPLOAD_BYTES) {
    return NextResponse.json(
      {
        success: false,
        error: 'La carga combinada del original y el procesado excede el limite permitido.',
        statusCode: 413,
      },
      { status: 413 },
    );
  ...
  try {
    response = await fetchBackend(`/documents/capture-sessions/${encodeURIComponent(token)}/upload`, {
      method: 'POST',
      body: formData,
